...
# Improvement #1: Warm client reuse confirmed; instatiated at top
...
# Improvement #2: Selective column loading for speed and memory efficiency
usecols = ["Diet_type", "Protein(g)", "Carbs(g)", "Fat(g)"]
df = pd.read_csv(io.BytesIO(stream), usecols=usecols)

# Improvement #3: Normalize Diet_type for consistent grouping
df["Diet_type"] = df["Diet_type"].astype(str).str.strip().str.title()

# Improvement #4: Fill only numeric NaNs safely and efficiently
